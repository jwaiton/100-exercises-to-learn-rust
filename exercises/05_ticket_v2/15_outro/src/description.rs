...
impl TryFrom<String> for TicketDescription {
    type Error = ParseTicketError;

    fn try_from(value: String) -> Result<Self, Self::Error> {
        if value.is_empty() {
            return Err(ParseTicketError { error: "The description cannot be empty".to_string() })
        }
        if value.len() > 500 {
            return Err(ParseTicketError { error: "The description cannot be longer than 500 bytes".to_string() })
        }
