    ...
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(err)   => {
            match err {
                ...
                TicketNewError::Description {message} => Ticket::new(title.clone(), "Description not provided".to_string(), status.clone()).unwrap()
            }
        }// conditionals on the error in here 
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::Title{message : "Title cannot be empty.to_string()".to_string()})
        }
        if title.len() > 50 {
            return Err(TicketNewError::Title{message : "Title cannot be longer than 50 bytes".to_string()})
        }
        if description.is_empty() {
            return Err(TicketNewError::Description{message : "Description cannot be empty".to_string()})
        }
        if description.len() > 500 {
            return Err(TicketNewError::Description{message : "Description cannot be longer than 500 bytes".to_string()})
        }
